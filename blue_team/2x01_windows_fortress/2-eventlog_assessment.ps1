...
foreach ($def in $eventDefinitions) {
    ...
    $tableOutput += [PSCustomObject]@{
        ...
        "Audit Subcategory" = $def.AuditSubcategory
        "Status"            = $status
    }

    $auditStatusVal = if ($auditConfig.ContainsKey($subcat)) { $auditConfig[$subcat] } else { "No Auditing" }
