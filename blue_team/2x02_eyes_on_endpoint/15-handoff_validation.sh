...
GT_TOTAL_ACTIONS=$(jq '
    ...
    elif (.actions // []) then ((.actions // []) | length)
...
if [[ "$WIN_TS_INVALID" == "null" ]] || [[ -z "$WIN_TS_INVALID" ]]; then WIN_TS_INVALID=0; fi
if [[ "$LIN_TS_INVALID" == "null" ]] || [[ -z "$LIN_TS_INVALID" ]]; then LIN_TS_INVALID=0; fi
...
if [[ "$WIN_FUTURE" == "null" ]] || [[ -z "$WIN_FUTURE" ]]; then WIN_FUTURE=0; fi
if [[ "$LIN_FUTURE" == "null" ]] || [[ -z "$LIN_FUTURE" ]]; then LIN_FUTURE=0; fi
...
if [[ -n "$ALL_GT_ACTIONS" ]]; then
    ...
    while IFS= read -r action_desc; do
        ...
        WIN_MATCH=$(jq --arg a "$action_desc" '[.matrix[]? // .detection_matrix[]? | select(.action == $a or .description == $a)] | length' "$WINDOWS_MATRIX" 2>/dev/null || echo "0")
        LIN_MATCH=$(jq --arg a "$action_desc" '[.matrix[]? // .detection_matrix[]? | select(.action == $a or .description == $a)] | length' "$LINUX_MATRIX" 2>/dev/null || echo "0")
