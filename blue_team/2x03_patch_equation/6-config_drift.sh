...
produce_diff() {
    ...
    if [[ -f "$dist_file" ]]; then
        diff -u "$dist_file" "$filepath" 2>/dev/null | head -n 40 || true
        return
    fi

    local old_file="${filepath}.dpkg-old"
    if [[ -f "$old_file" ]]; then
        diff -u "$old_file" "$filepath" 2>/dev/null | head -n 40 || true
        return
    fi

    local ucf_new="${filepath}.ucf-new"
    if [[ -f "$ucf_new" ]]; then
        diff -u "$ucf_new" "$filepath" 2>/dev/null | head -n 40 || true
        return
    fi

    local ucf_old="${filepath}.ucf-old"
    if [[ -f "$ucf_old" ]]; then
        diff -u "$ucf_old" "$filepath" 2>/dev/null | head -n 40 || true
        return
    fi

    diff -u /dev/null "$filepath" 2>/dev/null | head -n 40 || true
}
...
perform_drift_detection() {
    ...
    while IFS= read -r entry; do
        ...
        path=$(echo "$entry" | jq -r '.path // .file // ""')
        pre_hash=$(echo "$entry" | jq -r '.sha256 // ""')
        owning_pkg=$(echo "$entry" | jq -r '.package // .owning_package // ""')
    ...
    local pre_paths
    pre_paths=$(echo "$pre_conffiles_json" | jq -r '.[].path // .[].file' | sort -u)
