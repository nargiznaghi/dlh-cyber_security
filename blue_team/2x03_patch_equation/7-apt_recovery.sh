    ...
    for lf in "${LOCK_FILES[@]}"; do
        if [[ -f "$lf" ]]; then
            ...
            if command -v fuser >/dev/null 2>&1; then
                fuser "$lf" >/dev/null 2>&1 && is_held=true
            elif command -v lsof >/dev/null 2>&1; then
                lsof "$lf" >/dev/null 2>&1 && is_held=true
            fi

            if [[ "$is_held" == false ]]; then
...
run_dpkg_configure() {
    local rc
    set +e
    env $APT_ENV dpkg --configure -a >/dev/null 2>&1
    rc=$?
    set -e
...
run_fix_broken() {
    local rc
    set +e
    env $APT_ENV apt-get --fix-broken install -y >/dev/null 2>&1
    rc=$?
    set -e
...
restart_affected_services() {
    ...
    if [[ -z "$broken_pkgs_str" ]] || [[ ! -f "$DEPS_MAP_FILE" ]]; then
        echo "$results"
        return
    fi
    ...
    while IFS= read -r svc; do
        ...
        if [[ "$should_restart" == "true" ]]; then
            ...
            info "$(printf '%-38s %s' "${svc}.service" "${active_state}")"
...
main() {
    ...
    info "$(printf '%-38s OK' 'remove stale locks')"
    ...
    info "$(printf '%-38s %s' 'dpkg --configure -a' "$configure_result")"
    ...
    info "$(printf '%-38s %s' 'apt-get --fix-broken install' "$fix_result")"
    ...
    info "$(printf '%-38s %s' 'dpkg --audit (re-run)' "$final_audit")"
