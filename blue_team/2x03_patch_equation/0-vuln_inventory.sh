...
validate_prerequisites() {
    ...
    for cmd in dpkg-query apt-cache apt-get jq awk; do
...
classify_severity() {
    ...
    awk -v val="$cvss" 'BEGIN {
        if (val >= 9.0) print "critical"
        else if (val >= 7.0) print "high"
        else if (val >= 4.0) print "medium"
        else if (val > 0) print "low"
        else print "none"
    }'
}
...
calculate_max_cvss() {
    ...
    while IFS= read -r cve; do
        ...
        max_cvss=$(awk -v m="$max_cvss" -v c="$cvss" 'BEGIN { print (c > m) ? c : m }')
