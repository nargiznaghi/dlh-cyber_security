...
acquire_lock() {
    printf "[*] Acquiring lock %s..." "$LOCK_FILE"

    eval "exec ${LOCK_FD}>\"$LOCK_FILE\"" || {
        echo " FAILED" >&2
        echo "ERROR: Cannot create lock file" >&2
        exit 2
    }
    ...
    if flock -n "$LOCK_FD"; then
        ...
        while true; do
            ...
            if [[ $waited -ge 120 ]]; then
                echo " FAILED" >&2
                echo "ERROR: Could not acquire lock after 120 seconds" >&2
                exit 2
            fi
        done
    fi
...
execute_patch_entry() {
    ...
    printf '[%d/%d] %-22s %-13s' "$rank" "$total" "$package" "$bucket"
    ...
    if ! wait_for_dpkg_lock; then
        echo "apt-get ... FAILED (dpkg lock busy)"
...
main() {
    ...
    [[ ! -f "$PLAN_FILE" ]] && { echo "ERROR: Patch plan not found: $PLAN_FILE" >&2; exit 1; }

    acquire_lock

    local plan_count
    plan_count=$(jq '.plan | length' "$PLAN_FILE" 2>/dev/null || echo 0)
    echo "[*] Loading plan: patch_plan.json (${plan_count} entries)"
